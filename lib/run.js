...
(function($){
  ...
  function run(/* route or a hash of routes, callback */){
    
    // If there's no arguments, run all!
    ...
    if( arguments.length == 1 && typeof route == "string" ){
      ...
      for( var i=0; i < routes.length; i++ ){
        ...
        if( "selector" in r ){  
          ...
          // has been run.
          var el = $(r.selector).filter(function(){
            return !$(this).data("run:"+r.selector)
          }).data("run:"+r.selector,true);
    ...
    var isPath = typeof route == "string" && route.charAt() == "/";
    if( isPath ){
      /**
   ...
   * use this method.
   */
  $.run.again = function(selectorOrElement,now){
    $(selectorOrElement).removeData("run:"+selectorOrElement)
    now && $.run(selectorOrElement)
  }
